...
class RegionProposalNetwork(nn.Module):
    ...
    def forward(self, images, get_bboxes, get_classes):
        ...
        get_bboxes_proj = project_bboxes(get_bboxes, self.width_scale_factor, self.height_scale_factor, mode='p2a')
        
        positive_anc_ind, negative_anc_ind, GT_conf_scores, \
        GT_offsets, GT_class_pos, positive_anc_coords, \
        negative_anc_coords, positive_anc_ind_sep = get_req_anchors(anc_boxes_all, get_bboxes_proj, get_classes)
...
class ClassificationModule(nn.Module):
    ...
    def forward(self, feature_map, proposals_list, get_classes=None):
        ...
        cls_loss = F.cross_entropy(cls_scores, get_classes.long())
...
class TwoStageDetector(nn.Module):
    ...
    def forward(self, images, get_bboxes, get_classes):
        ...
        positive_anc_ind_sep, GT_class_pos = self.rpn(images, get_bboxes, get_classes)
...
def calc_bbox_reg_loss(get_offsets, reg_offsets_pos, batch_size):
    assert get_offsets.size() == reg_offsets_pos.size()
    loss = F.smooth_l1_loss(reg_offsets_pos, get_offsets, reduction='sum') * 1. / batch_size
    return loss